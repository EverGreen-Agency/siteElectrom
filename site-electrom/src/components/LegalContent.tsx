...
export function LegalContent({ defaultTab = 'termos' }: { defaultTab?: 'termos' | 'privacidade' }) {
  const searchParams = useSearchParams();
  const tabParam = searchParams.get('tab');
  const [activeTab, setActiveTab] = useState<'termos' | 'privacidade'>(
    tabParam === 'privacidade' ? 'privacidade' : defaultTab
  );
      ...
      <div className="max-w-4xl mx-auto relative z-10 space-y-8">
        ...
        <div className="flex justify-center border-b border-white/10 pb-4 gap-4">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-full text-xs font-medium uppercase tracking-wider transition-all cursor-pointer ${
              activeTab === 'termos'
